  ...
  <table width="1100" border="1" cellpadding="0" cellspacing="0" align="center">
         ...
         <tr>
         	<td colspan="10" align="right">
         	  <a href="admin/News-listByCId?catelogId=${catelogId }&currentPage=1" target="main">首页</a>
         	  <a href="admin/News-listByCId?catelogId=${catelogId }&currentPage=${pm.getPreviousPage()}">上一页</a>
         	  <c:forEach var="page" begin="1" end="${pm.getTotalPage() }" step="1">
         	  	<c:choose>
         	  		<c:when test="${page == pm.currentPage }">
         	  			<a href="admin/News-listByCId?catelogId=${catelogId }&currentPage=${page }"><font color="red">${page }</font></a>
         	  		</c:when>
         	  		<c:otherwise>
	         	  		<a href="admin/News-listByCId?catelogId=${catelogId }&currentPage=${page }">${page }</a>
         	  		</c:otherwise>
         	  	</c:choose>
         	  </c:forEach>
         	  <a href="admin/News-listByCId?catelogId=${catelogId }&currentPage=${pm.getNextPage() }">下一页</a>
         	  <a href="admin/News-listByCId?catelogId=${catelogId }&currentPage=${pm.getLastPage() }">尾页</a>
         	  总页数：${pm.getTotalPage() }
         	</td>
         </tr>
         <tr><td colspan="10" align="center">该分类共有<font color="red">${pm.totalRecords }</font>个新闻</td></tr>
     </table>
     </form>
  </body>
</html>
