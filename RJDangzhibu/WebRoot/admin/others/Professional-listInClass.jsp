 		...
 		<c:if test="${! empty pm.list}">
  		...
  		<tr>
  			<td colspan="6" align="right">
  				<a href="admin/Professional-listInClass?currentPage=1&professionalId=${professionalId }">首页</a>
  				<a href="admin/Professional-listInClass?currentPage=${pm.getPreviousPage() }&professionalId=${professionalId }">上一页</a>
  				<c:forEach var="page" begin="1" end="${pm.getTotalPage() }" step="1">
  					<c:choose>
  						<c:when test="${page == pm.currentPage }">
  							<a href="admin/Professional-listInClass?currentPage=${page }&professionalId=${professionalId }"><font color="red" >${page }</font></a>
  						</c:when>
  						<c:otherwise>
  							<a href="admin/Professional-listInClass?currentPage=${page }&professionalId=${professionalId }">${page }</a>
  						</c:otherwise>
  					</c:choose>
  				</c:forEach>
  				<a href="admin/Professional-listInClass?currentPage=${pm.getNextPage() }&professionalId=${professionalId }">下一页</a>
  				<a href="admin/Professional-listInClass?currentPage=${pm.getLastPage() }&professionalId=${professionalId }">尾页</a>
  				总页数：${pm.getTotalPage() }
  			</td>
  		</tr>
  	</table>
