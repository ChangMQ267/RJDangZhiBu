  	...
  	<form action="admin/UserInfo-batchDel" method="post" onsubmit="return checkIsNull()">
 		...
 		<tr>
         	<td colspan="10" align="right">
         	  <a href="admin/UserInfo-listByIId?inClassId=${inClassId }&currentPage=1" target="main">首页</a>
         	  <a href="admin/UserInfo-listByIId?inClassId=${inClassId }&currentPage=${pm.getPreviousPage()}">上一页</a>
         	  <c:forEach var="page" begin="1" end="${pm.getTotalPage() }" step="1">
         	  	<c:choose>
         	  		<c:when test="${page == pm.currentPage }">
         	  			<a href="admin/UserInfo-listByIId?inClassId=${inClassId }&currentPage=${page }"><font color="red">${page }</font></a>
         	  		</c:when>
         	  		<c:otherwise>
	         	  		<a href="admin/UserInfo-listByIId?inClassId=${inClassId }&currentPage=${page }">${page }</a>
         	  		</c:otherwise>
         	  	</c:choose>
         	  </c:forEach>
         	  <a href="admin/UserInfo-listByIId?inClassId=${inClassId }&currentPage=${pm.getNextPage() }">下一页</a>
         	  <a href="admin/UserInfo-listByIId?inClassId=${inClassId }&currentPage=${pm.getLastPage() }">尾页</a>
         	  总页数：${pm.getTotalPage() }
         	</td>
         </tr>
         <tr><td colspan="10" align="center">该班级共有党员:<font color="red">${pm.totalRecords }人</font></tr>
  	</table>
  </body>
</html>
