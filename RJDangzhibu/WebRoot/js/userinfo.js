...
function checkStudentName(obj) {
	if(isNull(obj)) {
		getById("studentNameError").innerHTML = "<font color='red'>学生姓名不能为空</font>";
	} else {
		getById("studentNameError").innerHTML = "";
	}
}
...
function check() {
	...
	var temp3 = getById("studentName").value;
	var flag = true;
	if(isNull(temp)) {
		getById("usernameError").innerHTML = "<font color='red'>用户名不能为空</font>";
		flag = false;
	...
	if(isNull(temp3)) {
		getById("studentNameError").innerHTML = "<font color='red'>用户姓名不能为空</font>";
		flag = false;
	}
	if(temp1 != temp2) {
		getById("submitError").innerHTML = "<font color='red'>两次密码不一致</font>"
		flag = false;
	}
	return flag;
}
function checkModify() {
	var temp = getById("username").value;
	var temp1 = getById("password").value;
	var temp2 = getById("studentName").value;
	var flag = true;
	if(isNull(temp)) {
		getById("usernameError").innerHTML = "<font color='red'>用户名不能为空</font>";
		flag = false;
